...
def getMObject(object):
	...
	if not mc.objExists(object):
		raise NameError('Object "'+object+'" does not exist!!')
	# Get selection list
...
def getPosition(point):
	...
	if (type(point) == list) or (type(point) == tuple):
		if len(point) < 3:
			raise NameError('Invalid point value supplied! Not enough list/tuple elements!')
		pos = point[0:3]
		...
		# Unknown type
		if not pos:
			raise NameError('Invalid point value supplied! Unable to determine type of point "'+str(point)+'"!')
	else:
		raise NameError('Invalid point value supplied! Invalid argument type!')
...
def getMPointArray(geometry,worldSpace=True):
	...
	if geometry and not mc.objExists(geometry):
		raise NameError('Object "'+geometry+'" does not exist!')
	...
	if getMObject(geometry).hasFn(OpenMaya.MFn.kTransform):
		...
		except:	raise NameError('Object "'+geometry+'" contains no valid geometry!')
...
def group_old(control,groupType=1,center=True,orient=True):
	...
	if not range(3).count(groupType):
		raise NameError('Invalid groupType value supplied!!('+str(groupType)+')')
...
def unitConversion(plug,conversionFactor=1.0,conversionFactorSourcePlug='',plugIsSource=True,prefix=''):
	...
	'''
	# Check plug
	if not mc.objExists(plug): raise NameError('Plug '+plug+' does not exist!!')
	if not plug.count('.'): raise NameError('Object '+plug+' is not a valid plug (node.attr)!!')
	
	# Check conversionFactorSourcePlug
	if conversionFactorSourcePlug:
		if not mc.objExists(conversionFactorSourcePlug):
			raise NameError('Conversion factor source plug '+conversionFactorSourcePlug+' does not exist!')
