#include "matrix.h"



Matrix::Matrix(std::vector< std::vector<float> >& m)
{
	this->rowsCount = m.size();
	this->colsCount = m[0].size();
	this->mat = m;
}

Matrix::Matrix(int rows, int cols)
{
	this->rowsCount = rows;
	this->colsCount = cols;
	for(int i = 0;i <rows;i++)
	{
		for (int j = 0;j<cols; j++)
		{
			this->row.push_back(float(j));
		}
		this->mat.push_back( row );
	}
	//mat(row, std::vector<float>(col));
}

std::vector<float> Matrix::getRow( int row )
{
	return this->mat[row];
}

void Matrix::setValue(int row, int col, float val)
{
	this->mat[row][col] = val;
}

float Matrix::getValue(int row, int col)
{
	return this->mat[row][col];
}

Matrix::~Matrix()
{
}

Matrix Matrix::mult( Matrix m )
{
	std::vector< std::vector<float> > mRes;
	std::vector< float > tmpMatRow;
	for (int i = 0; i< this->rowsCount; i++)
	{
		for (int z = 0; z < this->mat[i].size();z++)
		{
			tmpMatRow.push_back( this->mat[i][z] );
		}
		std::vector< float > mRow;
		for (int j = 0; j< m.colsCount;j++)
		{
			std::vector< float > tmpRow;
			for(int k = 0;k<m.rowsCount;k++ )
			{
				tmpRow.push_back( m.getValue(k,j) );
			}
			float res = 0;
			for( int y = 0; y < tmpMatRow.size(); y++ )
			{
				res = res + ( tmpMatRow[y]*tmpRow[y] );
			}
			mRow.push_back( res );
		}
		mRes.push_back( mRow );
	}
	return Matrix( mRes );
}

Matrix Matrix::invert()
{
	Matrix mTmp( this->mat );
	Matrix identity = make_identity(this->rowsCount, this->colsCount);
	for (int i = 0; i < this->rowsCount; i++)
	{
		for (int j = 0; j < this->colsCount; j++)
		{
			mTmp.mat[i].push_back( identity.getValue( i, j ) );
		}
	}
	int i = 0;
	int first_non_zero = 0;
	for (int j = 0; j < this->colsCount; j++)
	{
		float zero_sum = check_for_all_zeros( mTmp, i, j, first_non_zero );

		if (zero_sum == 0.0)
		{
			if( j == colsCount ) return mTmp;
		}
		if (first_non_zero != i)
			mTmp = swap_row( mTmp, i, first_non_zero );

		for (int q = 0; q < this->rowsCount; q++)
		{
			if( q != i )
			{
				std::vector< float > scaled_row;
				for (int m = 0; m < mTmp.mat[i].size() ; m++)
				{
					scaled_row.push_back( mTmp.getValue(q, j) * mTmp.mat[i][m] );
				}
				std::vector< float > temp_row;
				for (int m = 0; m < scaled_row.size(); m++)
				{
					temp_row.push_back( mTmp.mat[q][m] - scaled_row[m] );
				}
				mTmp.mat[q] = temp_row;
			}
		}

		if ( (i == this->rowsCount) || ( j == this->colsCount))
			break;

		i = i+1;
	}

	for (int i = 0; i < this->rowsCount; i++)
	{
		std::vector< float > temp_row;
		for (int m = colsCount; m < mTmp.mat[i].size(); m++)
		{
			temp_row.push_back( mTmp.mat[i][m] );
		}
		mTmp.mat[i] = temp_row;
	}
	
	return mTmp;
}

Matrix Matrix::swap_row( Matrix X, int i, int p )
{
	std::vector< float > tmpI;
	std::vector< float > tmpP;
	tmpI = X.mat[i];
	tmpP = X.mat[p];
	X.mat[p] = tmpI;
	X.mat[i] = tmpP;
	return X;
}

float Matrix::check_for_all_zeros(Matrix X, int i, int j, int &first_non_zero)
{
	float zero_sum = 0;
	first_non_zero = -1;
	for (int m = i; m < X.rowsCount; m++)
	{
		bool non_zero = X.getValue(m,j)!=0.0;
		if (non_zero) zero_sum +=1.0;
		if ((first_non_zero == -1) && (non_zero))
		{
			first_non_zero = m;
		}
	}
	return zero_sum;
}

Matrix Matrix::make_identity( int rows, int cols )
{
	std::vector< std::vector<float> > matTmp;
	std::vector< float > rowTmp;
	for(int i = 0;i <rows;i++)
	{
		for (int j = 0;j<cols; j++)
		{
			float elem = 0.0;
			if (i == j) elem = 1.0;
			rowTmp.push_back( elem );
		}
		matTmp.push_back( rowTmp );
	}
	return Matrix(matTmp);
}
