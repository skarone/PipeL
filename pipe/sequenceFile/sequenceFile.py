...
class sequenceFile(fl.File):
	...
	def createMov(self, outdir = '', audioPath = '', fps = 24, extraText = '' ):
		...
		cmd = ' -r ' + str(fps) + ' -i "'+ self.dirPath + self.name + '.%04d.png" ' + audioCmd + ' -r '+ str(fps) + ' -y -start_number '+str(self.start)+ ' ' + cmdFrameNum + ' -qscale 0 -pix_fmt yuv420p \"'+  outdir + self.name +'.mov\"'
		#-vcodec mjpeg
		print cmd
