...
def exportAllFromAnim( projectName, sequenceName, shotName, serverPath = 'P:/' ):
	...
	cam = getCamera()
	if not len( cam ) == 1:
		print 'There are more than one camera or no camera... so I cant know what camera to export, delete other cameras or create one to export'
	else:
...
def getSetsInScene():
	...
	refs = mn.ls( typ = 'reference' )
	setsRefs = []
	for r in refs:
		if '_shading' in r.name.lower() or '_final' in r.name.lower():
			if not '_rig' in r.name.lower:
				setsRefs.append( r )
	return setsRefs
...
def exportSets( sets, sho ):
	...
	nods = []
	for s in sets:
		nods.extend( mc.referenceQuery( s, nodes = True ) )
	nods = mn.Nodes( nods )
	nods.select()
	maFile = mfl.mayaFile( sho.setsPath.path )
	maFile.newVersion()
